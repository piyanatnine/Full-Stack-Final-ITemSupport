...
function Header({click, show, user}) {
  ...
  const UserProfile = () => {
    return (
      <button className="flex" onClick={()=> {setDropDown(!dropDown)}}>
        <div class="rounded-full w-7 h-7 bg-black mx-2 overflow-hidden">
          <img className="object-cover w-7 h-7" src={user.img}/>
        </div>
        <span>
          { user.name}
        </span>
  ...
  return (
    ...
    > 
      ...
      <div className="grid justify-items-end px-5 content-center hover:bg-[#d8dce0]">
          <UserProfile/>
          {dropDown && <div id="dropdown" class="absolute mt-20 w-32 px-3 py-1 rounded-md shadow-lg bg-white focus:outline-none">
